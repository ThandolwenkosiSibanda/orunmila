...
const MyProjectApi = async (status) => {
	console.log('myprojects saga', status);
	return await axios.get('/api/myprojects', { params: { status } });
};
...
function* fetchMyProjects(action) {
	try {
		let { data } = yield call(MyProjectApi, action.payload);
